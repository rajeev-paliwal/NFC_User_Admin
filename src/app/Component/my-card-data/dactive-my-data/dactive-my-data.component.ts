...
@Component({
  selector: 'app-dactive-my-data',
  templateUrl: './dactive-my-data.component.html',
  styleUrls: ['./dactive-my-data.component.css']
})
export class DactiveMyDataComponent implements OnInit {
  ...
  DactivePost(Data: any ) {
    //debugger ;
   this.HolderName  =Data.ele.FirstName +' ' + Data.ele.MiddleName  +' ' + Data.ele.LastName;
   this.CardHolderId = Data.ele.CardHolderid;
   this.UniqueID = Data.ele.UniqId;
    this.crd_services.DeActivedCardData(this.CardHolderId , this.UniqueID).subscribe((res: any) =>{
      this.event.emit('OK');
